...
- **Prerequisites**: `backend: programmatic` needs an eval command that prints
  ...
  grant — never assumed. A deterministic scripted-execution capability and
...
2. **Interview until unambiguous** per
   [references/interview.md](references/interview.md): use the graphical or structured user-input tool in
...
3. **Write and approve the brief.** Create the run's
   ...
   <IMPORTANT>
   Hard gate: no subagent dispatch or scripted-execution launch happens
...
4. **Scaffold and calibrate.** Scaffold the run layout once per
   ...
   panel, one use of the graphical or structured user-input tool for the
...
5. **Execute the research loop** — rounds of Generate, Score, Verify, Evolve
   ...
   mechanism: A (deterministic scripted execution) when that capability is available and
