...
- NEVER rewrite merged-on-origin history without explicit consent. For a detected target, prompt through the graphical or structured user-input capability; default to the corrective-PR route in [workflow-correct-merged.md](references/workflow-correct-merged.md). `--allow-rewrite-merged` skips the prompt.
...
| `--allow-rewrite-merged` | Explicit consent to rewrite history already merged on origin (skips the graphical or structured user-input capability corrective-PR prompt). |
