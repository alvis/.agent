...
def test_action_logger_writes_jsonl_entries(tmp_path: Path) -> None:
    ...
    assert [
        (entry["event"], entry.get("page"), entry.get("issues")) for entry in entries
    ] == [
...
def test_browser_driver_logs_successful_action(
    ...
    assert {key: entries[-1][key] for key in ("event", "action", "success")} == {
