...
- This is the only skill that authors Theriety-dialect `.mdc` body content.
  `notion-sync` may materialize transport files or update pairing metadata through
  `specification:sync-notion`, but callers must not hand-edit MDC.
- Specification callers bind this skill exactly as
  `--body-author=specification:mdc` and pass that selector unchanged through the
  operation chain. Accept only the already approved body and exact path; never
  select identity, transport, or authority here.
...
6. After all changed MDC files pass review, invoke the generic transport owner
   once in read-only metadata mode:

   `specification:sync-notion validate-metadata path/to/a.mdc path/to/b.mdc`

   That operation must not change file bytes or interpret body grammar. Compare
   its reported values to the pre-edit capture and reject any local change to
   transport-owned metadata. Store local edit timestamps, if useful, only in
   work evidence/receipts. Investigate any refusal.
